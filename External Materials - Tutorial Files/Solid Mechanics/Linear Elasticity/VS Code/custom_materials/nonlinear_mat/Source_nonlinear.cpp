...
EXPORT int eval(double S[6],         // Input: Green-Lagrange strain tensor components in Voigt order (xx,yy,zz,yz,zx,xy)
	...
	// Check inputs
	if (nPar[0] != 3)       // now three parameters needed, E0, E1, and nu
	...
	// Determine nonlinear stiffness 
	dV = S[0] + S[1] + S[2];		//volume dilation 
	E = E0 + E1 * abs(dV);

	// Check values of input parameters
	if (E <= 0.0) return -1;		//returning any value expcept '0' indicates an error. 
